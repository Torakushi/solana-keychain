...
`solana-keychain` is a Rust library providing a unified interface for signing Solana transactions across multiple backend implementations. The architecture centers around a single `SolanaSigner` trait that abstracts over ten different signing backends: Memory (local keypairs), Vault (HashiCorp), Privy, Turnkey, AWS KMS, Fireblocks, GCP KMS, CDP (Coinbase), Para, and Dfns.
...
cd rust && cargo test --features cdp
cd rust && cargo test --features para
...
8. **CdpSigner** ([rust/src/cdp/mod.rs](rust/src/cdp/mod.rs))
   - Uses Coinbase Developer Platform API
   - JWT-based authentication with wallet-scoped signing
   - Public key derived from provided Solana address
   - Availability checked via list wallets endpoint

9. **ParaSigner** ([rust/src/para/mod.rs](rust/src/para/mod.rs))
   - Uses Para MPC wallet API for signing
   - Requires `init()` to fetch wallet public key before use
   - Availability checked via wallet GET endpoint

10. **DfnsSigner** ([rust/src/dfns/mod.rs](rust/src/dfns/mod.rs))
...
- `cdp` - Adds CdpSigner with reqwest, jsonwebtoken
- `para` - Adds ParaSigner with reqwest, hex
...
Tests are co-located with implementation code in each module. Remote signers (Vault, Privy, Turnkey, AWS, Fireblocks, GCP, CDP, Para, DFNS) use `wiremock` to mock HTTP endpoints, avoiding actual API calls during testing. Tests cover:
...
cd rust && cargo test --features cdp cdp::tests
cd rust && cargo test --features para para::tests
...
- PrivySigner, FireblocksSigner, ParaSigner, and DfnsSigner must call `init()` before use; other signers are ready after construction
